...
def printGridToFile(grid, hiddenWords):
    '''
    :param grid: the grid created by createGrid
    :param hiddenWords: the words that are added to the grid
    :return: None. but create an output file with the wordsearch and list of words
    '''
    outfile=open("WordSearch.txt","w")
    for row in range(len(grid)):
        for column in range(len(grid[row])):
            outfile.write(grid[row][column]+" ")
        outfile.write("\n") # go to next line

    outfile.write("\n\n\nThis is the list of words to find in the word search\n\n")

    for j in range(len(hiddenWords)):
        outfile.write(hiddenWords[j]+ "\n")
...
def fillGrid(grid):
    ...
    for current_row in range(len(grid)):
        for current_col in range(len(grid[0])):
            if grid[current_row][current_col]==".":
                randlet=random.randint(ord("A"),ord("Z"))
                grid[current_row][current_col]=chr(randlet)
...
def main():
    ...
    for i in range(len(words)):
        placeWord(grid,words[i].upper())

    # fill in the grid
    fillGrid(grid)

    # Print it out
    printGridToFile(grid, words)
